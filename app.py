...
if url_input:
    pdf_links = [link.get("href") for link in BeautifulSoup(requests.get(url_input).content, "html.parser").find_all("a") if link.get("href", "").endswith(".pdf")]
    for link in pdf_links:
        if st.button(f"Process {link.split('/')[-1]}"):
            response = requests.get(link)
            with io.BytesIO(response.content) as pdf_file:
                process_pdf(pdf_file, link.split("/")[-1])
...
if stored_files:
    ...
    st.write("No files stored in the database.")
